...
clear
echo "----------------------------------------"
echo "macosHigh Sierra ISO GENERATOR FOR VMWARE ESXi"
echo "----------------------------------------"
...
# Mount the installer image from the macOS High Sierra Install App

echo "Mount the installer image from the macOS High Sierra Install App"
echo "----------------------------------------------------------"
echo ""
...
# Create the macOS High Sierra Blank ISO Image of 8500mb with a Single Partition - Apple Partition Map
...
echo "Restore the Base System into the macosHigh Sierra Blank ISO Image"
echo "----------------------------------------------------------"
echo ""
...
# Copy macOS High Sierra installer dependencies

echo "Copy macOS High Sierra installer dependencies"
echo "----------------------------------------------------------"
echo ""
...
echo "Unmount the macos High Sierra ISO Image"
echo "----------------------------------------------------------"
echo ""
...
hdiutil resize -size `hdiutil resize -limits /tmp/macosHighSierra.sparseimage | tail -n 1 | awk '{ print $1 }'`b /tmp/macosHighSierra.sparseimage
 
# Convert the macosHigh Sierra ISO Image to ISO/CD master

echo "Convert the macos High Sierra ISO Image to ISO/CD master"
echo "----------------------------------------------------------"
echo ""
hdiutil convert /tmp/macosHighSierra.sparseimage -format UDTO -o /tmp/macosHighSierra.iso
echo ""


# Rename the macOS High Sierra ISO Image and move it to the desktop

echo "Rename the macOS High Sierra ISO Image and move it to the desktop"
echo "----------------------------------------------------------"
echo ""
mv -v /tmp/macosHighSierra.iso.cdr ~/Desktop/macosHighSierra.iso
echo ""
...
echo "----------------------------------------------------------"
echo ""
rm /tmp/macosHighSierra.sparseimage
echo ""
echo "----------------------------------------------------------"
